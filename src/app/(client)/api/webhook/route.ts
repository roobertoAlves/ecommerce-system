...
export async function POST(req: NextRequest) {
  ...
  if (!webhookSecret) {
    return NextResponse.json({ error: "Webhook secret is not defined" }, { status: 400 });
  }
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const invoice = session.invoice
      ...
      : null;
    const initialStatus = session.payment_status === "paid" ? "paid" : "pending";
    try {
  ...
  if (event.type === "checkout.session.async_payment_succeeded") {
    ...
    try {
      ...
      return NextResponse.json({ error: `Error updating order: ${error}` }, { status: 400 });
    }
  }
...
async function createOrderInSanity(
  ...
  const order = await backendClient.create({
    ...
    invoice: invoice
      ? { id: invoice.id, number: invoice.number, hosted_invoice_url: invoice.hosted_invoice_url }
      : null,
  ...
  for (const { productId, quantity } of stockUpdates) {
    try {
      ...
      if (!product || typeof product.stock !== "number") {
        console.warn(`Product ${productId} not found or stock is not a number.`);
        continue;
      }
