      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            role="listbox"
            aria-label={t("regionCurrency")}
            className="absolute right-0 top-full mt-2 w-72 bg-surface rounded-xl border border-border shadow-xl z-[200] overflow-hidden"
          >
              ...
              {allConfigs.map((cfg) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => { setCurrency(cfg.currency as SupportedCurrency); setOpen(false); }}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-lg text-left transition-colors duration-150 w-full ${
                      isActive
                  ...
                  >
                    ...
                    {isActive && <span className="ml-auto w-2 h-2 rounded-full bg-primary shrink-0" />}
                  </button>
                );
              })}
            </div>
            <div className="px-4 py-2.5 border-t border-border bg-bg-secondary">
              <p className="text-[11px] text-text-muted">{t("pricesAdjust")}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
