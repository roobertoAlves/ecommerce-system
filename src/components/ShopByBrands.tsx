...
const ShopByBrands = async () => {
  ...
  return (
    <div className="mb-10 lg:mb-20 bg-bg-secondary p-5 lg:p-7 rounded-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-16 p-2 shadow-sm hover:shadow-primary/20 py-5">
        {extraData?.map((item, index) => (
          <div
            key={index}
            className="flex items-center gap-3 group text-text-muted hover:text-primary"
          >
            <span className="inline-flex scale-100 group-hover:scale-90 hoverEffect">
              {item?.icon}
            </span>
            <div className="text-sm">
              <p className="text-text-primary font-bold capitalize">
                {item?.title}
              </p>
