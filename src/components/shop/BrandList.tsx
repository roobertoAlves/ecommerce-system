...
const BrandList = ({ brands, selectedBrands, setSelectedBrands }: Props) => {
  ...
  return (
    <div className="w-full p-5">
      <Title className="text-base font-black text-text-primary">{t("brands")}</Title>
      <div className="mt-2 space-y-1">
        {brands?.map((brand) => {
          const slug = brand.slug?.current as string;
          const isSelected = selectedBrands.includes(slug);
          return (
            <div key={brand._id} onClick={() => toggle(slug)} className="flex items-center space-x-2 cursor-pointer group">
